...
class Persianmagento_Zarinpalzar_Block_Success extends Mage_Core_Block_Template 
{
	protected function _toHtml() {
		...
		if ($Status == 'OK') {
			...
			if ( (!$client) OR ($err = $client->getError()) ) {
				$this->_order->addStatusToHistory ( Mage_Sales_Model_Order::STATE_CANCELED, Mage::helper ( 'zarinpalzar' )->__('Could not connect to bank or service.'), true );
				$this->_order->save ();											
		  		...
		  		if ($client->fault){            	
					...
					if ($err) {
						...
						if($verify_result['Status'] == 100){																
										...
										$order->addStatusToHistory ( $this->_paymentInst->getConfigData ( 'second_order_status' ), Mage::helper ( 'zarinpalzar' )->__( Mage::Helper('zarinpalzar')->getBankMessage($verify_result['Status'])) . " " . $message, true );			
										$order->save ();							
			...
			$msg = Mage::Helper('zarinpalzar')->getBankMessage($verify_result['Status']);
			$this->_order = Mage::getModel ( 'sales/order' )->loadByIncrementId ( $saleOrderId );												
			$this->_order->addStatusToHistory ( Mage_Sales_Model_Order::STATE_CANCELED, Mage::helper ( 'zarinpalzar' )->__( $msg), true );
			$this->_order->save ();
